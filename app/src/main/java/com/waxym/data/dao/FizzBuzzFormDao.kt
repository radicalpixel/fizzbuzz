...
import com.waxym.data.model.FizzBuzzForm.Companion.COL_COUNT
...
interface FizzBuzzFormDao {
    ...
    @Query("SELECT * FROM $TABLE_NAME ORDER BY $COL_COUNT DESC LIMIT :limit")
    suspend fun getMostFrequent(limit: Int): List<FizzBuzzForm>

    @Query("SELECT SUM($COL_COUNT) FROM $TABLE_NAME")
    suspend fun sumCount(): Int
